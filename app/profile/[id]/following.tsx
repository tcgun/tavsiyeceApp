...
export default function FollowingScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setIsLoading(true);
      try {
        // Tüm takip edilenleri çek
        const followingList = await getFollowing(viewedUserId, 100);
        setFollowing(followingList);

        // Mevcut kullanıcının bu takip edilenleri takip edip etmediğini kontrol et
        if (currentUserId) {
        ...
        >
          {following.length > 0 ? (
            ...
            <View style={styles.emptyContainer}>
              <Text style={[styles.emptyText, mutedTextStyle]}>Henüz kimseyi takip etmiyor.</Text>
            </View>
          )}
        </ScrollView>
      )}
